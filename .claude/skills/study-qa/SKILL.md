...
- **화면 렌더도 비용이 난다.** 엔진이 클라우드인 언어의 화면은 마운트 시 `prefetchSpeech()`가 `/api/tts` 합성을 최대 `PREFETCH_MAX_ITEMS`(90)개 보낸다. 기본 엔진은 `lib/speech.ts` `DEFAULT_ENGINE`이 정한다 — 영어(en-US: 카드·챕터 리더·단어장·시험·오답노트, 토익 표현 카드·시험·모의고사 학습 보기·응시 결과·응시 시작 탭의 지시문·질문)와 한국어(ko-KR: 해설 낭독의 해설 조각, 운동 세션 음성 안내)가 클라우드다. 운동은 `▶ 운동 시작` 탭에서 휴식 뒤 안내 문구 4개(휴식은 세트 사이에만 — 2~5세트 풀업)를 프리페치하고, 휴식이 끝날 때마다 `speakQueue`로 한 문장을 읽는다. 일본어(ja-JP)는 기본이 기기 음성이라 `prefetchSpeech`가 아무것도 보내지 않고, 사용자가 일본어 엔진을 클라우드로 바꿨을 때만 `/api/tts`를 부른다. 키를 비우면 501 → 기기 음성으로 떨어져 0원이 된다. **토익 모의고사 학습 보기는 열리자마자 pending 사진 두 장을 자동 생성 요청**한다(관문 P, 이미지 생성) — 키를 넣은 서버에서 이 화면을 열면 발음보다 비싼 호출이 나간다. **은우 자유대화**는 화면을 여는 것만으로는 비용이 없지만(발음 프리페치 없음) 📞 탭 한 번이 Realtime 통화(분 단위 과금)와 주제 일러스트 1장이고, **개발 전용 가짜 전송도 `/connect`·`/scene`을 실제로 부른다** — 자유대화 e2e는 키를 비우는 대신 루프백 스텁(`OPENAI_API_KEY=sk-stub OPENAI_BASE_URL=http://127.0.0.1:<포트>/v1`)에 묶는다(`references/english.md` §8).
