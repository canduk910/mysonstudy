...
호출 옵션이 세 파일에 흩어져 있다 — A·A′·B·F는 `lib/ai/client.ts`, C·D·H는 `vocabbook-prompts.ts`, G는 `prompts.ts`. 옵션을 바꾸면 스펙도 함께 고친다. 스펙 §1 표는 A·A′·B·C만 싣는다. D는 §8-6, F는 §9-6, G는 §10-5, H는 §11-6에 값이 있다.
...
| 필수 유형·순서 | 픽션 7종·논픽션 6종 필수, 마지막은 나와연결 | `[questions]` | **없음** | **없음** | **AR<2 픽션은 질문이 6개인데 필수 유형이 7종이다.** 프롬프트 안에서 지킬 수 없는 조합이다. 픽스처(Pooh AR 2.0)가 이 경로를 밟지 않아 드러난 적이 없다. AR<2 쪽 다이얼을 만질 때는 이 모순부터 사용자와 정리하라 |
...
|---|---|---|---|
| `EVAL_OFFLINE_ONLY=1` | 오프라인 점검 전부. 2026-09-24 기준 153항목: 호출 A′ 스키마 15 · 분량 다이얼 9 · 낭독 자막 8 · 챕터화 30 · 단어뜻 11 · 유의어추천 12 · 단어장 §7 57(병합·zod·보강·시험 보기·관계 문제·오답 집계·복습) · 프롬프트↔스펙 11 | 0 — `globalThis.fetch`까지 막는다 | 표 + spec-sync 상세 출력 후 return |
...
- **스펙과 코드가 어긋난 곳이 있다.** 스펙 §5는 `EVAL_TRANSCRIPT=1`을 "자막 카드 1회 추가"라고 쓰지만, 코드에서는 게이트가 기본 카드를 대체한다. 비용은 코드 기준으로 계산하고, 어긋남은 리포트에 남겨 스펙 갱신을 요청하라.
...
분기 커버리지: 두 권 모두 AR≥2라 단어 12·질문 8 경로다. 픽션/논픽션 분기(funFacts null vs 4개)는 두 권이 나눠 맡는다. **AR<2 경로(단어 10·질문 6)는 픽스처가 커버하지 않는다.** 게다가 §2의 모순(AR<2 픽션: 질문 6개 · 필수 유형 7종)이 이 경로에 있다. AR<2 다이얼을 바꿀 때는 AR 1점대 임시 메타로 수동 스팟 체크를 1회 하고(실호출 1회, 비용 승인 필요) 결과를 리포트에 남긴다.
