...
- 은우 영어의 세 번째 메뉴 **자유대화** — 주제(프리셋 10개·직접 입력)나 단어장 하나를 고르면 AI 전화영어 선생님 **Sunny**와 최대 5분 **실시간 음성**으로 이야기합니다(OpenAI Realtime, 브라우저 ↔ OpenAI WebRTC — 키는 서버에만). 대화 중에는 선생님·은우 말이 화자별 **실시간 스크립트**로 쌓이고, 은우가 막히면 "이렇게 말해 볼까요?" 답 예시·단어 카드가 뜨며(5초 조용하면 자동 + 🙋 버튼), 선생님이 말하는 사물은 이모지 **그림 카드**로, 주제 장면은 AI **일러스트** 한 장으로 보입니다(카드는 소리를 내지 않는다 — 마이크가 열려 있다). 끝나면 문장을 탭해 선생님 말투의 설명(한국어 + 영어, 자동 낭독)을 듣습니다. 13세 미만 음성이 OpenAI를 거치는 것(기본 설정에서 남용 감시로 최대 30일 보관)은 보호자가 알고 진행하기로 했고, 앱은 **음성을 저장하지 않고** 글자 스크립트만 가족 DB에 둡니다(`docs/SPEC.md` §21-1). 어른 동반이 전제입니다. 상세는 `docs/SPEC.md` §21·`docs/harness/english.md` §12.
...
- **아빠 영역**(학습자가 아이가 아니라 아빠) — **아빠의 일본어**: JLPT 단어장(레벨·주제로 AI 생성, 후리가나·시험 5모드(뜻→표기·한자→읽기·표기→뜻·빈칸·일일정의→표기)·오답노트, 한자 시험 2모드), 듀오링고 대화 스크린샷 → 전사·학습 해설(해설을 한국어 설명까지 이어 읽는 **해설 듣기**), 한자 학습(한국 한자음을 다리로). 상세는 `docs/harness/japanese.md`·`docs/SPEC.md` §18. **아빠의 영어**(토익스피킹): 표현 암기장 사진을 판독하거나 준비한 파일로 가져와 표현마다 **발화 포인트**(문항별 활용 문장·답변 틀·바꿔 쓰기·발음·이어 말하기)를 붙인 카드·전체 듣기·시험 4모드(뜻→표현·표현→뜻·빈칸·소리 내어 말하기)·오답노트, 그리고 AI가 새로 만든 11문항 **모의고사**(Q3–4 사진은 AI 생성) → 실전 형식 응시(질문 음성·타이머·녹음 — 녹음은 기기에만) → 버튼을 눌러야 도는 **AI 채점**(음성 전사 + 피드백, Q1–2는 지문 대조)·추정 등급. 교재 내용은 공개 저장소에 넣지 않고 앱의 "파일로 가져오기"로만 넣습니다. 상세는 `docs/harness/toeic.md`·`docs/SPEC.md` §20. **아빠의 운동**: 러시안 파이터 풀업·푸시업 사다리 — RM을 넣으면 27일 사이클을 계산하고, 오늘 세트·슈퍼세트 세션(휴식 타이머·음성 안내)·실패하면 회복 휴식 하루 뒤 직전에 성공한 Day 목표로 한 번 → 성공하면 실패한 Day 재도전·Day 27 재측정 → 새 사이클 자동 계산. 계산은 **LLM을 쓰지 않는** 순수 엔진입니다(세션 음성 안내만 발음 엔진을 거친다). **총 운동 소요시간**도 봅니다 — ▶ 운동 시작 세션이 처음 연 순간부터 마지막 기록까지를 재고(실측), 세션 없이 기록한 날·지난 기록은 횟수·세트·기본 휴식으로 어림합니다(근사, "약 12분"). 상세는 `docs/SPEC.md` §19(소요시간 §19-8).
- 화면 공통: **언어별 발음 엔진**(기본은 영어·한국어 OpenAI TTS, 일본어 기기 음성 — 사용자가 언어별로 바꿀 수 있고, 클라우드가 실패하면 기기 음성으로 조용히 폴백 — §16)과 모든 화면 상단의 **학습 스트릭**(은우 · 아빠 일본어 · 아빠 영어 · 아빠 운동의 연속일을 나란히 — 은우는 단어장 시험과 한마디라도 한 자유대화를 센다, §17).
- 스택: Next.js(App Router, TS) · Tailwind CSS · OpenAI Responses API(Structured Outputs) · OpenAI TTS(`gpt-4o-mini-tts`) · OpenAI 이미지 생성(`gpt-image-2`)·음성 전사(`gpt-4o-mini-transcribe`) · OpenAI Realtime(`gpt-realtime-2.1`, WebRTC) · Google Books/Open Library · Firestore · Cloud Run · Supadata(자막)·YouTube Data API v3(낭독 영상 검색)
...
| `OPENAI_IMAGE_MODEL` | 선택 | 사진 생성 모델. 기본값 `gpt-image-2`. 아빠의 영어 모의고사 Q3–4 사진(관문 P, `lib/toeic-image.ts` — 1536×1024 JPEG, `toeicImages` 문서 하나)과 은우 자유대화 주제 일러스트(`lib/talk-image.ts` — 1024×1024, 품질 low 고정, `talkImages` 문서 하나)가 같은 공용 코어(`lib/image-gen.ts`)로 만든다(data URL 900,000자 상한) |
| `OPENAI_IMAGE_QUALITY` | 선택 | 토익 사진 품질 `low`·`medium`·`high`·`auto`. 기본값 `medium` — 모르는 값도 `medium`으로 되돌린다(그대로 보내면 400이라 사진이 조용히 사라진다). 자유대화 일러스트는 이 값을 보지 않고 `low` 고정(대화가 시작된 뒤 수십 초 안에 와야 쓸모가 있다) |
...
| `OPENAI_REALTIME_TRANSCRIBE_MODEL` | 선택 | 자유대화 중 은우 발화 전사 모델. 기본값 `gpt-4o-mini-transcribe`. 은우가 한국어를 섞으므로 `language`·`prompt`를 주지 않는다 |
...
| 자유대화는 글자만, 그림은 문서를 나눠 | `talkSessions`(스크립트·주제·단어 스냅샷·설명·보인 그림 카드)에 음성·녹음은 넣지 않는다 — 13세 미만 음성을 가족 DB에 두지 않는다(SPEC §21-1). 주제 일러스트는 `talkImages` 한 장 = 문서 하나(Firestore 문서 1MB)로 떼고, 대화와 한 원자 단위로 함께 만들고 지운다(삭제는 prod-guard `deleteTalkSession` 하나). 은우가 한마디도 안 한 대화는 저장하지 않고 그 그림도 서버에 남기지 않는다. 은우 단어장 컬렉션과 섞지 않는다 |
| 대화 저장은 멱등 — 저장 키가 곧 문서 id | 화면이 대화 한 번에 UUID(`clientSessionId`) 하나를 만들고 스토어가 그 값을 대화·그림 문서 id로 쓴다. 같은 키 + 같은 시작 시각이면 이미 저장된 대화를 돌려준다 — 응답 유실 뒤 "다시 저장"·화면 복귀 재시도·뒤로가기 정리가 겹쳐도 하나(QA가 응답만 잃게 해 대화 2건이 생기는 것을 재현했다). 별도 필드 + 조회는 Firestore 쿼리 트랜잭션·인덱스가 필요해, 파일은 `mutate` 하나·Firestore는 `batch.create`(ALREADY_EXISTS면 읽어서 판정)로 확인과 생성을 원자적으로 했다. 상한(턴 200·글자 1,000)을 넘으면 거부하지 않고 앞에서부터 잘라 저장한다 |
| 운동 소요시간 — 실측은 클라이언트 보고값, 근사는 저장하지 않음 | 사건에는 끝낸 시각 하나뿐이라 지난 기록의 실제 시간은 되살릴 수 없다. 그래서 세션이 잰 `durationSec`만 사건에 싣고(서버가 알 수 없는 값이라 "사건 값은 서버 계산" 원칙의 유일한 예외 — 형식·범위만 검증, 3시간 밖은 기록은 받고 null), 근사치는 읽을 때 엔진이 계산한다 — 백필하려고 프로덕션 DB를 고치지 않는다(SPEC §19-8). 필드가 빠진 옛 번들의 요청은 400 대신 null(근사) |
...
| 자유대화 음성은 하네스 밖 관문 R | 실시간 음성 대화는 JSON이 아니라 음성·전사·도구 호출 이벤트라 `callWithSchema`·zod·재요청에 넣지 않는다 — 키 규약만 공유(키가 없으면 연결 라우트가 501). 브라우저 ↔ OpenAI는 WebRTC로 직접 잇고, 서버는 SDP 교환(`/v1/realtime/calls`)과 hangup만 표준 키로 중계해 키가 폰에 가지 않는다. 정확성은 다른 자리에서 잡는다 — 지시문 원문 spec-sync(`block-exact`), 세션 설정은 SDK GA 타입, 스크립트는 순수 리듀서, 도구 인자는 순수 검사 함수. 대화 뒤 문장 설명(호출 I)만 하네스 안이다 |
...
| 자유대화 중 앱은 소리를 내지 않는다 | 대화 내내 마이크가 열려 있어 앱이 🔊를 내면 선생님이 그 소리를 은우 말로 듣는다 — 도움 카드·그림 카드에는 🔊가 없다. 그래서 마이크는 녹음과 달리 재생과 캡처가 겹치는 예외 경로(`acquireMicStream`)라 iPhone 스피커 음량·에코를 실기기로 확인해야 한다(SPEC §21-5) |
| 대화 화면이 사라지면 대화도 끝낸다 | 화면 숨김(잠금·앱 전환)·`pagehide`·**뒤로가기**(가장자리 스와이프로 오버레이가 언마운트됨) 모두 즉시 끝내고(연결 닫기 → 마이크 놓기 → 서버 hangup `sendBeacon`) 저장을 보낸다 — 처음엔 뒤로가기에 정리가 없어 아이 마이크가 화면 없이 OpenAI로 계속 흘렀다(QA P1). 저장 본문은 keepalive 한도를 **바이트**로 재고, 문서가 내려가는 중이면 그림을 빼서라도 대화를 남긴다. "끝내기" 버튼과 5분 마무리 뒤 종료만 아직 받아 적는 중인 은우 말을 최대 2.5초 기다렸다 닫는다(말을 마치자마자 눌러도 마지막 말이 남게) — 숨김·뒤로가기·`pagehide`는 기다리지 않는다 |
| 응답 진행 중엔 요청을 보류, 질문 없는 도구 응답의 이어 말하기·도움 요청은 연속 2회까지 | Realtime은 한 번에 한 응답만 대화에 쓸 수 있어, 선생님 응답이 만들어지는 중의 도움 요청은 보류했다가 그 응답이 소리 없이 끝나면 보내고, 소리가 있었으면(선생님이 새 차례를 말했다) 버린다. 도구를 부르고 끝난 응답의 말에 **질문이 없으면**(도구만 부른 응답 포함) 이어 말하기를 보낸다 — 실연결(2026-09-26)에서 선생님이 "Let me think of a small next question for you."처럼 말하다 도구를 부르고 멈춰, 은우가 질문을 못 듣는 것을 확인했다. 이 이어 말하기는 은우가 말하기 전까지 연속 2회로 막는다 — 매 응답이 대화 전체를 다시 입력으로 과금하기 때문이다. 12초 도움 요청은 선생님 차례마다 한 번이고, 은우가 말하기 전까지는 **연속 2번**까지다(🙋 요청 합산 — 은우가 말을 시작하면 다시 센다, 2026-09-26 확정). 상한에 닿으면 도움 카드는 그대로 뜨고 요청만 멈춘다 — 전에는 은우가 계속 조용하면 선생님 차례마다 요청이 되풀이됐다 |
| 운동 소요시간은 세션을 **처음 연** 순간부터 | 첫 ✓ 전 새로고침·닫았다 다시 열기에도 시작이 유지되게 진행 보존값에 `startedAt`을 두고 0스텝도 저장한다(세션을 한 번 열면 오늘 카드가 "이어서 하기"가 된다). 끝 시각은 기록 요청을 처음 보낸 순간에 고정해 네트워크 오류 뒤 재시도로 늘지 않는다. 대가: 열어 보기만 해도 시계가 돈다(3시간 상한 안이면 걸러지지 않음). 시작점은 "처음 연 순간"으로 확정했다(2026-09-26 — 첫 ✓로 바꾸지 않는다, SPEC §19-8) |

## 6. 개발 하네스

이 저장소는 클로드 코드 하네스로 개발됩니다 — AI 호출 명세(프롬프트·스키마·검증·eval)는 과목별로 `docs/harness/english.md`(영어 북카드·단어장·은우 자유대화 — 자유대화의 실시간 음성 관문 R과 문장 설명 호출 I는 §12)·`docs/harness/math.md`(수학코치)·`docs/harness/japanese.md`(아빠의 일본어)·`docs/harness/toeic.md`(아빠의 영어 — 토익스피킹)에 있고(AI를 쓰지 않는 아빠의 운동은 `docs/SPEC.md` §19) 과목 공통 규약은 `docs/HARNESS.md`, 앱 전체 명세는 `docs/SPEC.md`, 에이전트·스킬 구성은 `.claude/`를 참조하세요.
