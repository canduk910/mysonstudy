...
[storyOutlineKo / storyIsGuess — 줄거리 미리보기]
- storyOutlineKo: 3~4문장. 아이가 읽기 전에 이야기 흐름을 대충 알고 따라갈 수 있게 우리말로 쉽게 쓴다.
- 결말은 직접 말하지 않는다. 마지막 문장은 결말이 궁금해지게 닫는다.
- 논픽션이면 줄거리 대신 책이 다루는 내용을 아이 눈높이로 순서대로 소개한다.
- 확실히 아는 책이면 일반적인 수준의 줄거리를 쓰고 storyIsGuess를 false로 한다.
  잘 모르는 책이면 제목·주제·표지·소개글로 예상한 줄거리를 쓰고 storyIsGuess를 true로 한다.
- [절대 규칙] 1·2는 여기에도 그대로 적용된다: 본문 문장을 옮기지 말고, 세부 서술이 아니라 큰 흐름만 쓴다.
