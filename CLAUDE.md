...
**트리거:** 앱 구현·수정, AI 모듈(lib/ai), 프롬프트 튜닝, eval 실행, QA·정합성 검증 등 이 프로젝트의 개발 작업 요청 시 `study-orchestrator` 스킬을 사용하라. 영어(북카드)든 수학(수학코치)든 일본어(아빠의 일본어)든 진입점은 하나다. 스펙 내용에 대한 단순 질문은 직접 응답 가능.
