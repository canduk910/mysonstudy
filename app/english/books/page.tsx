/**
 * 영어 북카드 홈 `/english/books` (IA 재편) — 서버 컴포넌트.
 *
 * **이 화면은 원래 `/english`였다.** IA 재편으로 `/english`가 "허브(고르기)"가 되면서, 북카드
 * 홈(사진으로 만들기·최근 카드)은 통째로 여기로 옮겼다(내용·동작 그대로). 상단 내비(과목 선택·
 * 서재)는 공통 셸(`app/english/layout.tsx`)이 담당하므로 여기서는 제목·리드만 남긴다. 예전에
 * 있던 "단어장 정복" 부차 링크도 허브·셸이 대신하므로 뺐다.
 *
 * 화면의 주인공은 큰 진입 버튼 2개(사진/제목)다 — HomeCreate(클라이언트)가 그린다. 여기서는
 * "최근 만든 카드" 3개를 store로 읽어 렌더한다.
 *
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-16 pt-6">
      <header className="mb-8">
        <h1 className="t-book-title">은우 북카드</h1>
