/**
 * POST /api/english/talk/reorder — 지난 대화 목록 수동 정렬 (SPEC §15-1·§21-2) — 범용 계약(lib/reorder-contract.ts) 재사용,
 * 스토어만 reorderTalkSessions. AI 없음. 없는 id는 스토어가 조용히 건너뛴다.
...
export async function POST(req: Request) {
  ...
  try {
    await getStore().reorderTalkSessions(parsed.data.orderedIds);
