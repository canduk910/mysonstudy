 ...
 * 일본어와 운동은 **각자의 트랙**이다(합치지 않는다). 수학·읽음·대화·생성은 제외.
...
import { getStore, type WorkoutCycleRecord } from "@/lib/store";
...
/** 운동 트랙 중립값 — 운동 계산이 실패해도 헤드라인 전체(은우·일본어)는 살린다 */
...
export async function GET() {
  ...
  const [vocab, jaVocab, jaKanji, workoutCycles] = await Promise.all([
  ...
  const body: StreakResponse = { ok: true, today, eunwoo, appa, appaWorkout };
