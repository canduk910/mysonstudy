/**
 * POST /api/toeic/mocks/reorder — 모의고사 목록 수동 정렬 저장 (docs/harness/toeic.md §8, SPEC §15-1)
 *
 * 표현집 `/api/toeic/sets/reorder`와 같은 골격 — 범용 계약(lib/reorder-contract) 재사용, 스토어 메서드만 reorderToeicMocks로
...
export async function POST(req: Request) {
  ...
  try {
    await getStore().reorderToeicMocks(parsed.data.orderedIds);
