 ...
 * 파랑(accent)은 강조 셋에만 — 보류 배너 / 1막의 "조심!" / 아빠 티칭 포인트.
 *
 * 클라이언트 전용 API를 쓰지 않으므로 "use client" 지시자가 없다(호출부가 클라이언트라
 * 함께 번들된다). `lib/ai/*`는 **타입만** import한다 — 값 import는 openai·API 키를
 * 클라이언트 번들로 끌고 들어온다.
 */
...
const SCENE_TIER_TEXT: Record<SceneTier, { ko: string; detail: string }> = {
  typed: {
  ...
  html: {
    ko: "만든 그림(2단)이 준비됐어요",
    detail: "이 문제는 전용 그림에 맞지 않아 AI가 직접 그림을 짰어요. (M2.5에서 표시돼요)",
  },
  none: {
  ...
  return (
    <section className="u-card mt-4 border-line-strong" role="status">
      ...
      <div className="p-4 sm:p-5">
        <h2 className="t-section-title">⚠️ 아빠가 한 번 확인해 주세요</h2>
  ...
  return (
    <details className="u-box mt-4">
      <summary className="t-list-title cursor-pointer">
        🔒 답은 접어 두었어요 — 아빠가 확인한 뒤 펼쳐 주세요
      </summary>
      <p className="t-caption mt-2">
        아래 두 답이 서로 다르거나 확인되지 않았어요. 어느 쪽이 맞는지 아빠가 봐 주세요.
      </p>
    ...
    <article>
      ...
      <header>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {held && <span className="u-chip border-accent bg-accent text-bg">⚠️ 아빠 확인 필요</span>}
        </div>
      ...
      <section className="mt-8">
        ...
        {/*
         * ┌ 되감기 플레이어가 붙을 자리 (M2 = 1단 전용 렌더러 / M2.5 = 2단 HTML iframe) ┐
         * │ M1은 그림을 그리지 않는다. `content.scene`(1단 대본)이 있어도 렌더하지 않고, │
         * │ 2단 HTML은 애초에 만들지 않는다(라우트가 호출 E 렌더러를 주입하지 않음).      │
         * │ 아래 sceneTier 표시가 "여기에 무엇이 붙을 수 있는지"를 남겨 두는 자리다.      │
         * │ iframe의 srcdoc 조립·sandbox·CSP는 player-builder 소관이다 — 배치만 여기.    │
         * └──────────────────────────────────────────────────────────────────────────┘
         */}
        <div className="u-box mt-3">
          <p className="t-list-title">🎬 움직이는 그림은 준비 중이에요</p>
          <p className="t-caption mt-1">{SCENE_TIER_TEXT[sceneTier].detail}</p>
          <p className="mt-2 flex flex-wrap gap-2">
            <span className="u-chip">그림 준비 상태: {sceneTier}</span>
            <span className="u-chip">{SCENE_TIER_TEXT[sceneTier].ko}</span>
          </p>
        ...
        {held && (
          <p className="t-caption mt-3">
            아래 풀이에 나오는 숫자도 아직 확인 전이에요. 아빠와 함께 짚어 보세요.
          </p>
        )}
