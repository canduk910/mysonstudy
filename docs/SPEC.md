...
3. 줄거리 미리보기 (storyOutlineKo 3~4문장, 결말 미공개 — 논픽션은 "내용 미리보기" 제목, storyIsGuess=true면 "예상" 배지. 필드가 없는 기존 카드는 섹션 생략)
4. STEP 1 읽기 전 워밍업 (2개)
5. STEP 2 필수 단어장 (12개, 표: 단어·한글발음 / 뜻 / 쉬운 영어 풀이 / 예문) + 아빠 티칭 포인트 박스
6. STEP 3 읽으면서 미션 (3개)
7. STEP 4 읽고 나서 대화 (8개, 유형 태그 + 영어 질문 + 한국어 질문 + 부모용 힌트)
8. [논픽션만] 아빠 찬스: 재미있는 사실 (4개, 영/한)
9. 확장 놀이 (2개)
...
interface Card {
  ...
  storyOutlineKo?: string;      // 줄거리 미리보기 3~4문장 (논픽션은 내용 소개, 결말 미공개). 신규 생성엔 필수 — 기존 저장 카드에는 없음
  storyIsGuess?: boolean;       // true = 제목·주제·표지·소개글 기반 예상 줄거리 ("예상" 배지 표시)
...
[줄거리 미리보기]
- storyOutlineKo 3~4문장(결말 미공개, 논픽션은 내용 소개). 잘 모르는 책은 예상 줄거리를 쓰고 storyIsGuess를 true로.
