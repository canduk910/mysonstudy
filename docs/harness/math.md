...
| **B. 설명 설계** | 문제 1개(+**사진**) → 3막 설명 + (있으면) 두 번째 풀이 + 답 + 장면 JSON + 채점 | 창작 품질 + 정확성 | 0.5 | ~6,500 |
| **C. 검산** | 문제 문장(+**사진**) → 답 재계산 | 정확성, 독립성 | 0 | ~2,500 |
...
## 11. 로드맵 — 무엇이 서 있고 무엇이 남았나

**이 절이 마일스톤의 유지처다.** 그전까지는 `app/math/page.tsx` 주석이 유일한 기록이었고,
그래서 M4(저장·기록)를 만들 때가 되어서야 §9가 통째로 비어 있는 것을 발견했다.
단계를 추가하거나 끝낼 때 여기를 갱신하라.

### 11-1. 서 있는 것

| 단계 | 내용 | 근거 절 |
|---|---|---|
| **M1** | 문제 직접 입력 → 호출 B→C→장면 검산 → `ok`/`held` | §3 · §4 · §5 |
| **M4** | 설명 저장·기록 — 서재·다시 보기·유형별 통계 | §9 |
| **M5** | 두 번째 풀이(`insight`) + 삼자 대조 | §10 |
| **M2.5** | 되감기 플레이어(호출 E) — 키트·iframe 격리·3초 폴백 | §3-4 |
| **M3** | 문제집 사진 판독(호출 A) — 업로드·판독 확인·고른 문제만 설명 | §2 · §8 |

### 11-2. 남은 것

| 단계 | 내용 | 준비된 것 | 남은 것 |
|---|---|---|---|
| **호출 D** | 연습문제 | 프롬프트 §6 · 스키마 **전부 있음** | 라우트 · 화면 |
| **1단 전용 렌더러** | `scene`(1단) 대본을 그리는 화면 | 타입 §3-3 · 검산 §4 · 대본 생성 **동작 중** | 렌더러 화면 — 지금은 `typed`여도 그림이 안 뜨고 안내만 나온다 |

> **1단 렌더러를 어느 유형부터 만들지는 §8[개정 4]가 정한다** — 서재 통계표의 그림 단계
> 집계(1단/2단/없음)에서 **HTML로 자주 그려지는 유형이 곧 다음에 만들 전용 렌더러다.**
> 추측으로 고르지 말고 쌓인 기록을 보라.

> **M3에 버킷은 필요 없었다(2026-08-19 구현으로 확인).** `app/math/page.tsx` 주석이 오래 "비공개 저장소(버킷)와 서명 URL,
> 새 환경 변수가 함께 있어야 한다"고 적어 두었는데 **과한 전제였다.** 영어 쪽(`/api/extract`·
> `/api/pages`)이 이미 같은 일을 버킷 없이 한다 — 클라이언트에서 리사이즈해 **base64 data URL**로
> 보내고, AI에 넘긴 뒤 **저장하지 않는다**. 저장소 어디에도 `Storage`·`bucket`·`signedUrl`이 없다.
> §9-1이 기록에 남기는 것도 판독된 **텍스트**(`problem`)이지 사진이 아니다.
> 재사용할 것: `lib/upload-limits.ts`(상한 단일 정의처) · `components/home-create.tsx`(리사이즈 UI).

### 11-3. 품질 과제 — 2026-08-19 실호출 eval이 알려준 것

통과 70 / 실패 9. **코드 결함은 0건이고 전부 프롬프트 다이얼이다.** `held` 0건(답은 4건 다 맞음).

| 항목 | 관찰된 것 |
|---|---|
| 장면 구조 | `split` 단계에 최종 답(600·300)을 넣었다 — 차를 뗀 뒤 남은 것을 나눈 300·300이어야 한다. `moves`가 비었는데 `rewind` 단계를 만들었다 |
| 통찰 판단 | `ruler-eraser`(합·차)는 §10-5가 통찰을 기대하는 문제인데 만들지 않았다 |
| `trap` 길이 | 40자 한도에 42·43자 — 경계만 살짝 넘는다 |
| 채점 | `partial` 기대인데 `wrong`으로 판정했다 |
| 2단 요청 | 1단으로 될 문제에 `sceneHtmlRequest: true`를 냈다 |

**장면 검산은 정상이다** — 같은 규칙이 손으로 만든 정답 대본은 통과시켰다(eval O2).
규칙이 맞고 모델 출력이 어긋난 것이며, 어긋난 장면은 버려지고 텍스트 3막은 살아남았다.

### 11-4. 아직 재지 못하는 것

| 공백 | 왜 |
|---|---|
| Firestore `explanations` 실동작 | 로컬은 파일 백엔드로만 검증한다(프로덕션 접속 금지). 첫 실사용이 곧 검증이다 |
| 생성 문장의 "엄빠" 호칭 | eval이 `storyOutlineKo`만 찍고 `hintKo`·`teachingTipKo`는 출력하지 않는다 |
| 억지 통찰 감시 | §10-5 (b) `absent` 갈래를 밟는 픽스처가 지금 0개다(§10-5 각주) |
| iframe `ready`(점검 9-G) | 브라우저 의존을 eval에 들이지 않기로 했다. 앱의 3초 폴백이 이 경우를 받는다 |

## 12. 문제 사진을 B·C에도 넘긴다 (M6)

### 12-1. 왜 — 도형 문제가 통째로 새고 있었다

지금까지 **사진을 보는 것은 호출 A 하나뿐**이었다. B(설명 설계)와 C(검산)는
`그림 설명: {figureDesc}` **한 줄**로 그림을 받았고, §2-1은 그마저 "짧게 설명한다"고 지시한다
(예시가 `"통 가·나·다 그림, 아래는 각각 5L"` 수준이다).

문장제에서는 그 한 줄로 충분하다. §0이 정한 이 앱의 표적이 **"문장을 식으로 옮기는 단계"**였고,
실측된 오답 세 건이 전부 문장제였기 때문이다. 도형은 §0 [개정 3]에서 `counting` 유형으로
**나중에 합류했는데, 그때 유형 코드와 렌더 지침만 늘리고 이미지 경로는 손대지 않았다.**

그 결과 점판 위 도형·겹친 도형·보조선·색칠된 영역처럼 **문장으로 옮겨지지 않는 그림**은
B에게 도달하지 못한다. 소마 사고력수학은 그런 문제가 대부분이다.
사용자가 실제로 도형 문제를 찍어 보고 "그림을 이해하지 못한다"고 보고했다(2026-08-21).

### 12-2. B와 C **둘 다** 받는다

C에 안 주면 **C가 구조적으로 불리해진다.** 그림을 본 B와 못 본 C의 답이 어긋나고,
§5-3이 그것을 `held`로 접는다 — 검산이 일하는 게 아니라 근거가 달라서 지는 것이다.
**독립 검산은 같은 근거 위에서만 공정하다.** 재시도(§5-3) 때도 같은 사진을 다시 보낸다.

**각주 — 사진에는 아이 필기와 채점 표시가 함께 실린다. 그리고 그것이 C에게도 보인다.**
§2-1 4번이 지시하듯 판독 사진에는 아이가 연필로 쓴 풀이·답과 어른이 빨간 펜으로 친 채점 표시가
함께 찍혀 있다(촬영 안내도 "지우지 않고 찍어 주세요"라고 요구한다). 즉 §5-1이 텍스트로는 절대
넘기지 않는 `childAnswer`·`childWork`가 **그림으로는 심판에게 도달한다.** 이것이 앵커링 방어의
유일한 구멍이고, `eval O5`는 텍스트 경로만 읽으므로 **이 구멍을 감시하지 못한다** — 사진 속 필기는
코드로 읽을 수 없다. 사진을 넘기는 코드에도 같은 경고를 남긴다(`lib/ai/math/pipeline.ts`
`buildVerifyUserParts()` 머리주석).

그럼에도 사진을 주기로 한 것은 이 절의 저울이 반대쪽으로 더 무겁기 때문이다 — 그림을 못 본 C는
도형 문제에서 거의 확실히 다른 답을 내고, 그 불일치는 검산이 일한 결과가 아니라 근거가 다른 결과다.
**되돌아갈 조건:** 아이가 쓴 답이 C를 끌어당기는 정황이 보이면(C가 틀린 아이 답을 그대로 반복하거나,
채점 표시가 동그라미인 문제에서만 C가 B에 유난히 잘 동의하는 식) 이 결정을 다시 연다.
그때 논할 선택지는 **문제 영역 크롭**과 **필기 영역 마스킹**이고, 둘 다 §12-3이 유보한 비용
(크롭 좌표가 어긋나면 도형이 잘린다)을 다시 지게 된다.

### 12-3. 페이지 전체를 보내고, 어느 문제인지 말로 지목한다

문제 영역만 잘라 보내는 편이 토큰은 싸다. 그러나 크롭 좌표를 A가 내주어야 하고
(스키마·프롬프트가 함께 늘어난다), **좌표가 조금만 어긋나면 도형의 일부가 잘려 나간다** —
그림이 필요해서 넣는 기능인데 그림을 훼손하는 실패 모드다.

그래서 **판독할 때 쓴 사진을 그대로** 보내고, 사용자 메시지의 `번호:` 필드가 어느 문제인지
지목한다. A가 이미 문제를 분리해 번호를 읽어 두었으므로 새로 만들 정보가 없다.

**다만 번호가 없을 수 있다.** `number`는 nullable이고(§2-3), 널이면 사용자 메시지에 `번호: -`가
나간다 — 페이지에 문제가 여럿이면 지목 수단이 사라진다. 그래서 프롬프트(§12-5)가 폴백을 하나 더
준다: 번호가 `-`이면 **[문제]의 문장과 같은 문제**를 찾는다. `text`는 널이 아니고(§2-3), 사람이
확인 화면에서 고쳤다면 사진보다 정확하다. 호출 C는 애초에 번호를 받지 않으므로(§5-1의 사용자
메시지 템플릿에 `번호:` 필드가 없다) **문장 대조가 C의 유일한 지목 수단이다** — C 블록이 번호가
아니라 문장을 말하는 이유가 이것이다.

### 12-4. 사진은 선택이다

`/math/new`(직접 입력)에는 사진이 없다. **이미지 없이 도는 경로가 그대로 살아 있어야 한다** —
필드는 nullable이고, 없으면 지금까지와 똑같이 텍스트만으로 돈다.

**저장은 여전히 하지 않는다**(§9-1). 기록에 남는 것은 판독된 텍스트이지 사진이 아니다.
따라서 저장된 기록을 나중에 "다시 만들기" 할 때는 사진이 없다 — 그때는 텍스트만으로 돈다.
사진을 저장하려면 비공개 버킷과 보존 정책이 필요하고, 그것은 이 절의 범위가 아니다.

### 12-5. 프롬프트에 더할 절

**호출 B**(§3-1) — `[이 아이에 대해]` 뒤, `[절대 규칙]` 앞:
...
**왜 "사진을 믿는다"가 아닌가.** 이 절의 초판(2026-08-21)은 B 블록에 「문제 문장과 어긋나면 사진을
믿는다」고 적었다. 그런데 확인 화면(§2-4)은 부모에게 **문장을 고치라고** 요구하고 "고친 내용으로
설명을 만들어요"라고 약속한다. 사진에 찍힌 것은 **고치기 전 원문**이다 — A가 5L을 6L로 잘못 읽어
부모가 되돌린 바로 그 숫자가 사진에 그대로 남아 있다. 사진을 우선하면 설명이 그 틀린 숫자 위에 서고,
**B와 C가 같은 사진을 보므로 §5-3 검산이 이 오류를 잡지 못한다** — 두 심판이 같은 잘못된 근거에서
일치해 `ok`가 난다. 수학 과목의 최악 실패(틀린 답이 통과)에 곧장 닿는 경로다.

그래서 역할을 가른다: **숫자·조건은 문장이, 그림은 사진이** 근거다. 사진을 넣은 목적(§12-1)은
문장으로 옮겨지지 않는 그림을 읽는 것이지 문장을 대체하는 것이 아니었다.
이 갈라짐의 대가는 **부모가 확인 화면을 대충 넘겼을 때** 판독 오류가 그대로 남는다는 것이다 —
그 방어는 사진이 아니라 확인 화면이 맡는다. 사람이 한 번 본 문장을 기계가 뒤집게 두면
확인 화면이 하는 일이 없어진다.

### 12-6. 비용

이미지 1장(긴 변 1,500px·`detail:"high"`)이 대략 1,000~1,500 입력 토큰이다.

| | 지금 | 사진 포함 |
|---|---|---|
| B 입력 | ~4,400 | ~5,600 |
| C 입력 | ~400 | ~1,600 |
| 문제 1개(재시도 없음) | ~4,800 | **~7,200** |
| 문제 1개(재시도 1회) | ~9,600 | **~14,400** |

§8의 3문제 상한이 이 증가분을 이미 감당한다(최악 ~43,000 입력 토큰).
**출력 토큰은 늘지 않는다** — 늘어나는 것은 입력뿐이다.

### 12-7. eval

`rect-count`(도형 세기)가 이 경로의 픽스처다. 사진 없이 도는 기존 경로가 **깨지지 않는 것**을
함께 확인한다 — 픽스처 대부분은 사진이 없고, 그것이 정상이다.
